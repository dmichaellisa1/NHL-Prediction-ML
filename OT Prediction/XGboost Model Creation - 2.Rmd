...
res_db %>% arrange(desc(auc))
#best is max_depth: 5 min_child: 25
...
may want to look at higher numbers too later
...
g_7
```
eta 0.1 at round 175 got us the best AUCPR
...
boost_pred_class[boost_preds >= oc$MaxProdNPVPPV$Global$optimal.cutoff$cutoff[1]] <- 1
...
```{r}
boost_pred_class2 <- rep(0, length(boost_preds))
boost_pred_class2[boost_preds >= 0.3333333] <- 1

t2 <- table(boost_pred_class2, actual = test_db$OT) # Create table
